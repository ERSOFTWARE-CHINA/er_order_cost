...
@Component({
    selector: 'material_requisition-list',
...
export class MaterialRequisitionListComponent implements OnInit {
    ...
    add() {
        ...
        this.router.navigateByUrl('/material_requisition/form');
    }

    update(id) {
        this.mrSrv.formOperation='update';
        this.mrSrv.initUpdate(id)
            .then(result => { this.mrSrv.materialRequisition = result.data;})
            .then(() => this.router.navigateByUrl('/material_requisition/form')).catch((error)=>
